    ...
    *,
    ...
    body = {
        ...
        "result": {
            ...
            "account_level": max(0, min(999, int(safe_result.get("account_level") or 0))),
            "invite_count": max(0, min(29, int(safe_result.get("invite_count") or 0))),
            "invite_remaining": max(0, min(29, int(safe_result.get("invite_remaining") or 0))),
            "invite_owner_match": bool(safe_result.get("invite_owner_match", False)),
            "invite_referrer_player_id": str(safe_result.get("invite_referrer_player_id") or "")[:128],
        },
    }
...
def _invite_link_only_check(invite_link: str) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        from tools.onelink_resolver import normalize_onelink_url, resolve_http, validate_invite_uri

        normalized = normalize_onelink_url(invite_link)
        uri, _trace = resolve_http(normalized, timeout=12.0, max_hops=8, force_probe=True)
        if not uri:
            raise RuntimeError("ONELINK_DIRECT_URI_NOT_FOUND")
        params, warnings = validate_invite_uri(uri)
        severe = [x for x in warnings if x.startswith("MISSING_PARAMS=") or x == "DEEP_LINK_VALUE_NOT_INVITE"]
        if severe:
            raise RuntimeError("ONELINK_INVITE_PARAMS_INVALID")
        ...
        return {
            ...
            "invite_count": 0,
            "invite_remaining": 29,
            "invite_owner_match": True,
            "invite_referrer_player_id": referrer_player_id,
        }
    except Exception as exc:
        code = str(getattr(exc, "code", None) or str(exc) or type(exc).__name__)[:80].upper().replace(":", "_")
        if not code or not all(c.isalnum() or c in "_-" for c in code):
            code = "INVITE_LINK_CHECK_FAILED"
        return {
            "ok": False,
            "credential_valid": False,
            "code": code,
            "stage": "INVITE_LINK",
            "retryable": code in {"DEVPLAY_TIMEOUT", "DEVPLAY_NETWORK_ERROR", "ONELINK_NETWORK_ERROR"},
            "elapsed_ms": (time.perf_counter() - started) * 1000.0,
            "account_login_ok": False,
            "game_session_ok": False,
            "friend_api_ok": False,
            "relationship_ready": False,
            "invite_owner_match": False,
        }


def _invite_target_account_check(email: str, password: str, invite_link: str) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        from mwoif_worker.heart_one import _login_and_session
        from mwoif.invite.snapshot import fetch_invite_member_snapshot
        from tools.onelink_resolver import normalize_onelink_url, resolve_http, validate_invite_uri

        cfg, auth, _session = _login_and_session(
            email=email,
            password=password,
            account_kind="invite-target-check",
            account_id=0,
            slot="INVITER_CHECK",
            event_cb=None,
        )
        snapshot = fetch_invite_member_snapshot(cfg, slot="INVITER_CHECK", auth=auth, event_cb=None)
        normalized = normalize_onelink_url(invite_link)
        uri, _trace = resolve_http(normalized, timeout=12.0, max_hops=8, force_probe=True)
        if not uri:
            raise RuntimeError("ONELINK_DIRECT_URI_NOT_FOUND")
        ...
        if not referrer_player_id:
            raise RuntimeError("REFERRER_PLAYER_ID_MISSING")
        identities = {str(snapshot.session.member_seq), str(getattr(auth, "mid", "") or "").strip()}
        identities.discard("")
        owner_match = referrer_player_id in identities or str(params.get("af_referrer_customer_id") or "").strip() in identities
        count = max(0, min(29, int(snapshot.friend_invite_count)))
        return {
            "ok": bool(owner_match),
            "credential_valid": True,
            "code": "INVITE_ACCOUNT_CHECK_OK" if owner_match else "INVITE_LINK_OWNER_MISMATCH",
            "stage": "INVITE_READINESS",
            "retryable": False,
            "elapsed_ms": (time.perf_counter() - started) * 1000.0,
            "account_login_ok": True,
            "game_session_ok": True,
            "friend_api_ok": True,
            "relationship_ready": True,
            "account_level": int(snapshot.session.current_lv),
            "invite_count": count,
            "invite_remaining": max(0, 29 - count),
            "invite_owner_match": bool(owner_match),
            "invite_referrer_player_id": referrer_player_id if owner_match else "",
        }
    except Exception as exc:
        code = str(getattr(exc, "code", None) or str(exc) or type(exc).__name__)[:80].upper().replace(":", "_")
        if not code or not all(c.isalnum() or c in "_-" for c in code):
            code = "INVITE_ACCOUNT_CHECK_FAILED"
        stage = "LOGIN" if code.startswith("DEVPLAY_") else ("GAME_SESSION" if code.startswith("INITMEMBER3") else "INVITE_LINK")
        return {
            "ok": False,
            "credential_valid": stage != "LOGIN",
            "code": code,
            "stage": stage,
            "retryable": code in {"DEVPLAY_TIMEOUT", "DEVPLAY_NETWORK_ERROR", "INITMEMBER3_NETWORK_ERROR"},
            "elapsed_ms": (time.perf_counter() - started) * 1000.0,
            "account_login_ok": stage != "LOGIN",
            "game_session_ok": stage not in {"LOGIN", "GAME_SESSION"},
            "friend_api_ok": False,
            "relationship_ready": False,
            "invite_owner_match": False,
        }
    ...
    if work_type in {"DEVPLAY_CHECK", "RECEIVER_CAPACITY_PREP", "ACCOUNT_LOGIN_CHECK"}:
        ...
        try:
            if work_type == "DEVPLAY_CHECK":
                control_payload = job.get("control_payload") if isinstance(job.get("control_payload"), dict) else {}
                service_code = str(control_payload.get("service_code") or "").strip().upper()
                invite_link = str(control_payload.get("invite_link") or "").strip()
                invite_mode = str(control_payload.get("invite_mode") or "full").strip().lower()
                if service_code == "INVITE_PUMP" and invite_mode == "link_only":
                    safe_result = _invite_link_only_check(invite_link)
