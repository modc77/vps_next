...
class SenderSessionPool:
    ...
    def __init__(self) -> None:
        ...
        self.preferred_limit = _env_int("MWOIF_SENDER_SESSION_PREFERRED_LIMIT", 500, 10, 5000)
    ...
    def _decode_file(self, path: Path, sga_id: int) -> SenderSessionEntry | None:
        ...
        try:
            ...
            if auth is None or session is None or not auth.refresh_token:
                return None
    ...
    def _purge_locked(self, now: float) -> None:
        ...
        for sga_id, entry in self._entries.items():
            ...
            if (
                ...
                or not entry.auth.refresh_token
    ...
    def lab_refresh_snapshot(self) -> list[tuple[int, AuthRecord, SessionRecord, str]]:
        now = time.monotonic()
        with self._lock:
            self._purge_locked(now)
            return [
                (int(entry.sga_id), entry.auth, entry.session, str(entry.email_fp or ""))
                for entry in self._entries.values()
                if entry.auth.ready and entry.auth.refresh_token and entry.session.established
            ]

    def lab_replace_auth(self, sga_id: int, auth: AuthRecord, *, source: str = "refresh") -> bool:
        if sga_id < 1 or not auth.ready or not auth.refresh_token:
            return False
        now_mono = time.monotonic()
        now_wall = int(time.time())
        with self._lock:
            ...
            entry.last_used_wall = now_wall
            entry.source = str(source or "refresh")[:32]
            self._entries.move_to_end(int(sga_id), last=True)
            self._persist_entry_locked(entry)
            return True

    def lab_entry_state(self, sga_id: int) -> dict[str, object] | None:
        now = time.monotonic()
        with self._lock:
            self._purge_locked(now)
            entry = self._entries.get(int(sga_id))
            if entry is None:
                return None
            expiries = [jwt_exp(entry.auth.game_access_token), jwt_exp(entry.auth.oven_access_token)]
            known = [int(exp) for exp in expiries if exp is not None and int(exp) > 0]
            return {
                "sga_id": int(entry.sga_id),
                "remaining_seconds": (min(known) - int(time.time())) if known else None,
                "refresh_present": bool(entry.auth.refresh_token),
                "session_established": bool(entry.session.established),
